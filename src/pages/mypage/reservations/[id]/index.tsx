...
import { CANCELED, COMPLETED, CONFIRMED, DECLINED, PENDING, ReservationStatus } from '@/constants/reservation';
  ...
  return (
    <div className={styles.status}>
      {status === CANCELED || status === DECLINED ? (
        ...
        <>
          <div className={status === PENDING ? styles.active : styles.inactive}>{ReservationStatus['pending']}</div>
          <ArrowRightIcon alt="예약 신청에서 예약 승인으로 가는 화살표" />
          <div className={status === CONFIRMED ? styles.active : styles.inactive}>{ReservationStatus['confirmed']}</div>
          <ArrowRightIcon alt="예약 승인에서 체험 완료로 가는 화살표" />
          <div className={status === COMPLETED ? styles.active : styles.inactive}>{ReservationStatus['completed']}</div>
        </>
      )}
    </div>
  );
